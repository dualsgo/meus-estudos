...
class VirtualPet:
    # O método construtor se parece com __init__() e nos permite definir valores exclusivos para as variáveis de classe quando criamos uma instância. Para adicionar essa flexibilidade às nossa classes, começamos adicionando a função de construção, que se parece com __init__().
    
    def __init__(self, color):
        # Assim como acontece com os métodos de classe regulares, precisamos adicionar self como o primeiro parâmetro do método construtor. A seguir, adiconamos os parâmetrôs para os valores personalizados que queremos definir ao criar uma instância, como acontece com color aqui.
...
# Quando criamos uma instância a partir da definição de classe, podemos passar valores únicos entre parênteses, como aocntece com rocky e benny aqui.
