...
# Um literal são dados cujos valores são determinados pelo próprio literal. Em outras palavras, um literal representa um valor fixo, não variável. Por exemplo, a string "Olá" é um literal de string, enquanto a variável nome pode ser um literal de string ou um literal numérico, dependendo do valor que ela contém.
...
# Os números manipluados pelos computadores podem ser de dois tipos:
# inteiros, ou seja, aqueles que são desprovidos da parte fracionária (int)
...
# 0x 123 é um número hexadecimal com um valor (decimal) igual a 291. A função print() também pode gerenciar esses valores.
print(0x123)
...
# Assim utilziaremos a função format() para converter um número decimal em octal ou hexadecimal:
print(format(83, 'o'))
# Basta especificar a base como segundo argumento da função format(). 'o' para octal e 'x' para hexadecimal.

# 2.2.3 Floats

# Valores float são aqueles que contêm uma parte fracionária. Eles são escritos com um ponto decimal como separador entre a parte inteira e a parte fracionária. Por exemplo:
...
# O Python pode usar um apóstrofo em vez de uma aspas. Qualquer um desses caracteres pode delimitar cadeias de caracteres, mas você deve ser coerente. Se você abrir uma sequência com aspas, será necessário fechá-la com aspas.

print('Eu gosto "Monty Python"') # Eu gosto "Monty Python"

# Também é possível usar barras invertidas para escapar de aspas dentro de uma string. Ou aspas triplas para strings de várias linhas.
...
# Essas literais não são tão óbvias quanto as anteriores, pois são usadas para representar um valor muito abstrato - a veracidade. O nome vem do matemático George Boole, que foi o primeiro a definir um sistema de álgebra booleana, que é a base da lógica moderna e da computação que faz uso de apenas dois valores: verdadeiro e falso denotados como 1 e 0, respectivamente.
