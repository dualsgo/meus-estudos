...
# Um operador é um símbolo da linguagem de programação, capaz de operar com os valores. Por exemplo, assim como na aritmética, o sinal de + (mais) é o operador que é capaz de adicionar dois números, dando o resultado da adição.
...
# ** (dois asteriscos) - eleva um valor ao poder de outro
...
# O resultado da divisão do número inteiro é sempre arredondado para o valor inteiro mais próximo que é menor que o resultado real (não arredondado). Isso é muito importante: o arredondamento sempre vai para o número inteiro menor.
...
# Restante (módulo)
# Sua representação gráfica em Python é o sinal de % (percentual), que pode parecer um pouco confuso. O resultado do operador é o restante após a divisão do número inteiro. Em outras palavras, é o valor que falta depois de dividir um valor por outro para produzir um quociente inteiro.
...
# 3.0 – não 3 mas 3.0. A regra ainda funciona: 12 dividido por 4.5 dá 2.6666666666666665. O quociente é 2. O produto de 2 e 4.5 é 9. O restante é 12 - 9 = 3.0. O resultado é um float porque um dos operandos é um float.
