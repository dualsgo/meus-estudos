...
# Utilizamos os parênteses para envolver o texto que queremos exibir na tela.
...
# Há também um terceiro componente que pode ser usado para modificar o comportamento de uma função: o argumento.
# As funções matemáticas geralmente levam um argumento. Por exemplo, sin(x) leva um x, que é a medida de um ângulo.

# As funções Python, por outro lado, são mais versáteis. Dependendo das necessidades individuais, eles podem aceitar qualquer número de argumentos ‒ quantos forem necessários para realizar suas tarefas. Nota: quando dizemos qualquer número, que inclui zero ‒ algumas funções do Python não necessitam de argumento, como por exemplo print() que mesmo ao ser chamada sem argumentos imprime uma linha em branco. Ou seja, ela causa um efeito sem precisar de um argumento.
...
# Como você pode ver, a string é delimitada por aspas - na verdade, as aspas
# formam a string - elas cortam uma parte do código e atribuem um significado
# diferente a ele. Você pode imaginar que as citações dizem algo como: "O texto entre nós não se trada de um trecho de código. Não se destina a ser executado e você deve tomá-lo como está." Quase tudo o que você coloca dentro das aspas será levado literalmente, não como código, mas como dados.
...
print("A pequenina aranha escalou a tromba d'água."), print(
...
# 1. Se você quiser colocar apenas uma barra invertida numa string, não se
# esqueça de sua natureza de escape - você precisa dobrá-la. Por exemplo, uma invocação como esta causará um erro: print("\")
...
# a função print() coloca um espaço entre os argumentos gerados por sua
# própria iniciativa. Este efeito é causado pelo argumento sep (separador) que por padrão, é um espaço.
...
print("Meu", "nome", "é", "Monty", "Python.", sep="-")  # O resultado é: Meu-nome-é-Monty-Python.
